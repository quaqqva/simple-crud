...
public static class FilterExpressionGenerator<TArgument>
{
    ...
    private static readonly Dictionary<string, ExpressionOperationInfo> rpnOperationInfos =
        new()
        {
            [" and "] = new ExpressionOperationInfo { Cost = 1, Type = OperationType.Binary },
            [" or "] = new ExpressionOperationInfo { Cost = 1, Type = OperationType.Binary },
            ["not "] = new ExpressionOperationInfo { Cost = 2, Type = OperationType.Unary }
        };

    private static readonly string propertiesRegex = @"\w+(\.\w+)*(\s*[+\-*/]\s*\w+(\.\w+)*)*";

    private static readonly string expressionRegex =
        propertiesRegex + @"\s+(\S+)\s+([0-9]{2}\.[0-9]{2}\.[0-9]{4}|\d+|'((?!').)*')";

    public static Expression<Func<TArgument, bool>> Truthy => arg => true;

    public static Expression<Func<TArgument, bool>> GenerateFilterExpression(
        string filterString
    )
    {
        ...
        return RpnExpressionComputer.Compute(
            ...
            (operatorType, operand1, operand2) =>
                Expression.Lambda<Func<TArgument, bool>>(
                    operatorType.Trim() switch
                    {
    ...
    private static Expression<Func<TArgument, bool>> GenerateSimpleFilterExpression(
    ...
    )
    {
        ...
        try
        {
            ...
            var operatorAndValue = simpleExpression[
                    (propertyMatch.Index + propertyMatch.Length)..
                ]
                .Split(' ', StringSplitOptions.RemoveEmptyEntries);
            var operatorName = operatorAndValue[0].ToPascalCase();
            value = string.Join(' ', operatorAndValue[1..]);

            var (propertyAccess, propertyType) =
                PropertyExpressionGenerator<TArgument>.ParseComplexProperty(
                    propertyName,
                    parameter
                );
            ...
            Expression operation = operatorName switch
            {
                ...
                "Contains"
                    ...
                    => Expression.Call(
                        propertyAccess,
                        typeof(string).GetMethod(operatorName, [typeof(string)])!,
                        constant
                    ),
                _
                    => throw new ArgumentException(
                        $"Operator '{operatorName.ToCamelCase()}' is not supported"
                    )
            };
        ...
        {
            throw new ArgumentException(
                "Filter expressions should include property info, operator and value, separated by space"
            );
        }
        ...
        {
            throw new ArgumentException(
                $"Conversion for type of field '{propertyName}' is not supported"
            );
        }
    }
}